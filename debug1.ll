declare i32 @getint()
declare i32 @getch()
declare float @getfloat()
declare i32 @getarray(ptr)
declare i32 @getfarray(ptr)
declare void @putint(i32)
declare void @putch(i32)
declare void @putfloat(float)
declare void @putarray(i32,ptr)
declare void @putfarray(i32,ptr)
declare void @_sysy_starttime(i32)
declare void @_sysy_stoptime(i32)
declare void @llvm.memset.p0.i32(ptr,i8,i32,i1)
declare i32 @llvm.umax.i32(i32,i32)
declare i32 @llvm.umin.i32(i32,i32)
declare i32 @llvm.smax.i32(i32,i32)
declare i32 @llvm.smin.i32(i32,i32)
@RADIUS = global float 0x4016000000000000
@PI = global float 0x400921fb60000000
@EPS = global float 0x3eb0c6f7a0000000
@PI_HEX = global float 0x400921fb60000000
@HEX2 = global float 0x3fb4000000000000
@FACT = global float 0xc0e01d0000000000
@EVAL1 = global float 0x4052c00000000000
@EVAL2 = global float 0x403e000000000000
@EVAL3 = global float 0x403e000000000000
@CONV1 = global float 0x406d200000000000
@CONV2 = global float 0x40affe0000000000
@MAX = global i32 1000000000
@TWO = global i32 2
@THREE = global i32 3
@FIVE = global i32 5
define float @float_abs(float %r0)
{
L0:  ;
    br label %L2
L2:  ;
    %r5 = fcmp olt float %r0,0x0
    br i1 %r5, label %L5, label %L6
L5:  ;
    %r7 = fsub float 0x0,%r0
    ret float %r7
L6:  ;
    ret float %r0
}
define float @circle_area(i32 %r0)
{
L0:  ;
    br label %L1
L1:  ;
    %r2 = load float, ptr @PI
    %r4 = sitofp i32 %r0 to float
    %r5 = fmul float %r2,%r4
    %r7 = sitofp i32 %r0 to float
    %r8 = fmul float %r5,%r7
    %r11 = mul i32 %r0,%r0
    %r12 = load float, ptr @PI
    %r13 = sitofp i32 %r11 to float
    %r14 = fmul float %r13,%r12
    %r15 = fadd float %r8,%r14
    %r18 = fdiv float %r15,0x4000000000000000
    ret float %r18
}
define void @error()
{
L0:  ;
    br label %L1
L1:  ;
    call void @putch(i32 101)
    call void @putch(i32 114)
    call void @putch(i32 114)
    call void @putch(i32 111)
    call void @putch(i32 114)
    call void @putch(i32 10)
    ret void
}
define i32 @float_eq(float %r0,float %r1)
{
L0:  ;
    br label %L2
L2:  ;
    %r6 = fsub float %r0,%r1
    %r7 = call float @float_abs(float %r6)
    %r8 = load float, ptr @EPS
    %r9 = fcmp olt float %r7,%r8
    br i1 %r9, label %L5, label %L6
L5:  ;
    ret i32 1
L6:  ;
    ret i32 0
}
define void @ok()
{
L0:  ;
    br label %L1
L1:  ;
    call void @putch(i32 111)
    call void @putch(i32 107)
    call void @putch(i32 10)
    ret void
}
define void @assert(i32 %r0)
{
L0:  ;
    br label %L2
L2:  ;
    %r3 = icmp eq i32 %r0,0
    %r4 = zext i1 %r3 to i32
    %r5 = icmp ne i32 %r4,0
    br i1 %r5, label %L5, label %L6
L5:  ;
    call void @error()
    br label %L7
L6:  ;
    call void @ok()
    br label %L7
L7:  ;
    ret void
}
define i32 @main()
{
L0:  ;
    %r42 = alloca [10 x float]
    br label %L1
L1:  ;
    %r0 = load float, ptr @HEX2
    %r1 = load float, ptr @FACT
    %r2 = call i32 @float_eq(float %r0,float %r1)
    %r3 = icmp eq i32 %r2,0
    %r4 = zext i1 %r3 to i32
    call void @assert(i32 %r4)
    %r5 = load float, ptr @EVAL1
    %r6 = load float, ptr @EVAL2
    %r7 = call i32 @float_eq(float %r5,float %r6)
    %r8 = icmp eq i32 %r7,0
    %r9 = zext i1 %r8 to i32
    call void @assert(i32 %r9)
    %r10 = load float, ptr @EVAL2
    %r11 = load float, ptr @EVAL3
    %r12 = call i32 @float_eq(float %r10,float %r11)
    call void @assert(i32 %r12)
    %r13 = load float, ptr @RADIUS
    %r14 = fptosi float %r13 to i32
    %r15 = call float @circle_area(i32 %r14)
    %r16 = load i32, ptr @FIVE
    %r17 = call float @circle_area(i32 %r16)
    %r18 = call i32 @float_eq(float %r15,float %r17)
    call void @assert(i32 %r18)
    %r19 = load float, ptr @CONV1
    %r20 = load float, ptr @CONV2
    %r21 = call i32 @float_eq(float %r19,float %r20)
    %r22 = icmp eq i32 %r21,0
    %r23 = zext i1 %r22 to i32
    call void @assert(i32 %r23)
    br label %L5
L5:  ;
    call void @ok()
    br label %L7
L7:  ;
    br i1 1, label %L10, label %L12
L10:  ;
    call void @ok()
    br label %L12
L12:  ;
    br i1 0, label %L15, label %L18
L15:  ;
    br i1 1, label %L16, label %L18
L16:  ;
    call void @error()
    br label %L18
L18:  ;
    br i1 0, label %L22, label %L21
L21:  ;
    br i1 1, label %L22, label %L23
L22:  ;
    call void @ok()
    br label %L23
L23:  ;
    call void @llvm.memset.p0.i32(ptr %r42,i8 0,i32 40,i1 0)
    %r43 = getelementptr [10 x float], ptr %r42, i32 0, i32 0
    store float 0x3ff0000000000000, ptr %r43
    %r45 = getelementptr [10 x i32], ptr %r42, i32 0, i32 1
    store i32 2, ptr %r45
    %r48 = getelementptr [10 x float], ptr %r42, i32 0
    %r49 = call i32 @getfarray(ptr %r48)
    br label %L24
L24:  ;
    %r93 = phi i32 [1,%L23],[%r84,%L27]
    %r92 = phi i32 [0,%L23],[%r87,%L27]
    %r51 = load i32, ptr @MAX
    %r52 = icmp slt i32 %r93,%r51
    br i1 %r52, label %L27, label %L28
L27:  ;
    %r54 = call float @getfloat()
    %r56 = load float, ptr @PI
    %r58 = fmul float %r56,%r54
    %r60 = fmul float %r58,%r54
    %r63 = fptosi float %r54 to i32
    %r64 = call float @circle_area(i32 %r63)
    %r66 = getelementptr [10 x float], ptr %r42, i32 0, i32 %r92
    %r67 = load float, ptr %r66
    %r69 = fadd float %r67,%r54
    %r71 = getelementptr [10 x float], ptr %r42, i32 0, i32 %r92
    store float %r69, ptr %r71
    call void @putfloat(float %r60)
    call void @putch(i32 32)
    %r76 = fptosi float %r64 to i32
    call void @putint(i32 %r76)
    call void @putch(i32 10)
    %r82 = sitofp i32 %r93 to float
    %r83 = fmul float %r82,0x4024000000000000
    %r84 = fptosi float %r83 to i32
    %r87 = add i32 %r92,1
    br label %L24
L28:  ;
    %r89 = getelementptr [10 x float], ptr %r42, i32 0
    call void @putfarray(i32 %r49,ptr %r89)
    ret i32 0
}
